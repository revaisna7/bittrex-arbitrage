 ...
 */
 module.exports = class Route {
 	...
 	getInputs() {
 		...
 		this.dust = 1-Config.get('dust');
		this.dust = this.dust < 0 ? 0 : this.dust > 1 ? 1 : this.dust;
 		// deviated prices
		...
		this.marketXBtcAvailable = this.currencyX.convertToBtc(this.speculate ? this.marketX.getPotentialQuantity(this.currencyY,this.priceX) : this.marketX.getQuantity(this.currencyY,this.priceX));
 		...
 		// minimumums
 		this.minBtcBalance = Math.min(this.currencyXBtcBalance,this.currencyYBtcBalance,this.currencyZBtcBalance)*this.dust;
	...
	trade() {
		if(Config.get('trade')) {
			...
			this.tradeX = this.currencyX.tradeTo(this.currencyY, this.isXBase ? this.inputX : this.outputX, this.priceX).execute();
			this.tradeY = this.currencyY.tradeTo(this.currencyZ, this.isYBase ? this.inputY : this.outputY, this.priceY).execute();
			this.tradeZ = this.currencyZ.tradeTo(this.currencyX, this.isZBase ? this.inputZ : this.outputZ, this.priceZ).execute();
